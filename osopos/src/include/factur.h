...
factur.h vers. 0.10, biblioteca para el uso del m�dulo de facturaci�n de OsoPOS
...
int read_config() {
  ...
  if (config != NULL) {         /* Si existe archivo de configuraci�n */
    b = buff;
    ...
    while (!feof(config)) {
      ...
      strncpy(buf, strtok(buff,"="), mxbuff);
        ...
        /* Busca par�metros de impresora y archivo de �ltima venta */
      if (!strcmp(b, "factura")) {
    ...
    while (!feof(config)) {
      ...
      /*      else if (!strcmp(b,"db.usuario")) {
        ...
        if (aux != NULL) {
          ...
          fprintf(stderr, "factur. Error de memoria en argumento de configuraci�n %s\n",
                  b);
      }
