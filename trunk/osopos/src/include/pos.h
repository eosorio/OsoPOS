/* Librería para OsoPOS. Cubierta por la licencia GPL */
/* Vea el archivo LEAME, README  y COPYING para más información */
/* pos.h 0.18-1 1999, E. Israel Osorio H.  */
#include <stdlib.h>
#include <string.h>
#include <stdio.h>
...
/*char nmdatos[mxbuff]; */	/* Archivo de última venta */
...
PGresult *Agrega_en_Inventario(PGconn *base, char *tabla, struct articulos art)
{
  ...
  sprintf(comando_sql,
          "INSERT INTO %s (codigo, descripcion, pu, descuento, cant, min, max, id_prov, id_depto, p_costo) VALUES('%s', '%s', %.2f, %.2f, %u, %u, %u, '%s', '%s', %.2f)",
...
PGresult *Modifica_en_Inventario(PGconn *base, char *tabla, struct articulos art)
{
  ...
  comando_sql = calloc(1,mxbuff);
  sprintf(comando_sql,
         "UPDATE %s SET descripcion='%s', pu=%.2f, descuento=%.2f, cant=%u, min=%u, max=%u, id_prov='%s', id_depto='%s', p_costo=%.2f WHERE codigo='%s'",
